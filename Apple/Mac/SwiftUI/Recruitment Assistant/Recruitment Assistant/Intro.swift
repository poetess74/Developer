        ...
        VStack {
            ...
            HStack {
                Button(action: {
                    ...
                    Alert(title: Text("테스트에 사용할 문제가 없습니다. "), message: Text("테스트에 사용할 문제 & 신상파일을 선택하여 주시기 바랍니다. 승인 버튼을 누를 경우 자동으로 문제파일을 선택하는 화면으로 전환되며 권한이 있는 사람만 문제파일을 지정할 수 있으므로 지원자일 경우 관계자에게 알려 주시길 바랍니다. "), dismissButton: .default(Text("승인"), action: { self.UserDB.status = "TestOFSelect" }))
                }
            ...
            HStack {
                Text("\(self.UserDB.TestFile == nil ? "현재 테스트에 사용할 문제 & 신상 파일이 지정되지 않았습니다. " : "")")
