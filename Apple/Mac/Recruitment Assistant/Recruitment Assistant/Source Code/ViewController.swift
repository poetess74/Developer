...
class ViewController: NSViewController {
    ...
    private var incorrectTries = 1
    private let trustToken = 3
    ...
    @IBAction func adminAuthentication(_ sender: NSButton) {
        adminAuthentication()
    }
    ...
    @IBAction func privacyAgree(_ sender: NSButton) {
        let alert = NSAlert()
        alert.messageText = "개인 정보 식별에 동의하셨습니다. "
        alert.informativeText = "이제 답안을 검토한 후 '제출' 버튼을 클릭하여 답안을 제출하여 주십시오."
    ...
    @IBAction func Donetest(_ sender: Any) {
        ...
        alert.informativeText = "부분 점수가 있으며 채점 후 최종 발표일에 면접 성적과 함께 반영될 예정입니다. \n이제 면접 시간을 잡아 주시기 바랍니다.\n※ 본 시험용 윈도우는 자동으로 닫힙니다. ※"
        alert.addButton(withTitle: "승인")
        alert.alertStyle = .informational
        alert.runModal()
    ...
    private func Login() {
        if (incorrectTries >= trustToken) {
            userID.stringValue = ""
            userPIN.stringValue = ""
            userName.stringValue = ""
            loginStatus.isHidden = false
            loginStatus.stringValue = "조회를 계속할 권한이 없으므로 관리자에게 문의하십시오. "
            userLogin.isEnabled = false
            userID.isEnabled = false
            userPIN.isEnabled = false
            userName.isEnabled = false
            userHelper.stringValue = "토큰을 갱신하기까지 개인정보 보호를 위하여 조회할 수 없습니다. "
            adminID.isHidden = false
            adminPW.isHidden = false
            adminLogin.isHidden = false
            self.view.window?.standardWindowButton(NSWindow.ButtonType.closeButton)!.isEnabled = false
        } else if (userPIN.stringValue.isEmpty && userID.stringValue.isEmpty && userName.stringValue.isEmpty && userLanguage.selectItem(withTag: 0)) {
            loginStatus.isHidden = false
            ...
            loginStatus.isHidden = false
            loginStatus.stringValue = "이름을 정확히 기재하여 주십시오. "
            userName.stringValue = ""
            ...
            let userTag = userID.stringValue + userPIN.stringValue
            if (userDB.userLogin(Name: userID.stringValue, Tag: userTag)) {
                ...
                loginStatus.isHidden = false
                loginStatus.stringValue = "인적 사항을 찾을 수 없습니다. (\(trustToken - incorrectTries)회 남음)"
                userID.stringValue = ""
                userPIN.stringValue = ""
                userName.stringValue = ""
                incorrectTries += 1
    ...
    private func Logout() {
        ...
        userHelper.stringValue = "먼저 본인 인적사항을 입력하고 '조회' 버튼을 눌러 인증하시기 바랍니다. "
    }
    
    private func adminAuthentication() {
        if (userDB.adminLogintry >= userDB.adminTrustlogin) {
            NSLog("EXC_BAD_ACCESS. Recruitment Assistant.app/MainView/NSViewController had terminated for security.")
            self.view.window?.close()
        } else if !(adminID.stringValue.isEmpty || adminPW.stringValue.isEmpty) {
            if (adminID.stringValue != userDB.adminID || adminPW.stringValue != userDB.adminPW) {
                userHelper.isHidden = false
                userHelper.stringValue = "아이디 또는 비밀번호가 다릅니다. \(userDB.adminTrustlogin - userDB.adminLogintry)회 남음"
    ...
    @objc private func updateTimer() {
        if (sec > 0) {
            ...
            Logout()
            stopTimer()
            loginStatus.stringValue = "제한 시간이 초과되어 0점 처리되었습니다. "
        }
