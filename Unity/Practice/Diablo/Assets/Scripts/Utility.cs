...
public static class Utility {
     public static IEnumerator animPlayOneShot(Animator source, string clipName, string stateName, string valueName, float animIndex, bool reset) {
         ...
         Debug.LogFormat("animPlayOneShot: {0} Changed Animator parameters - {1}: {2}, {3}: {4}",source.gameObject.name, valueName, source.GetFloat(valueName), stateName, source.GetBool(stateName));
