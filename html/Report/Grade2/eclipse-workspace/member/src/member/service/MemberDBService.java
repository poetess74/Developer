...
public class MemberDBService {
	...
	public void registerMember(Member member) {
		...
		try {
			...
			if (stmt.executeUpdate() != 1) {
				throw new RuntimeException("알 수 없는 오류로 사용자 등록에 실패하였습니다.");
			}
		} catch (SQLException e) {
			e.printStackTrace(); // dump to log
			throw new RuntimeException("데이터베이스 오류가 발생하였습니다.(SQL-ERROR=" + e.getErrorCode() + ")");
		} finally {
	...
	public void updateMember(Member member) {
		...
		try {
			...
			if (stmt.executeUpdate() != 1) {
				throw new RuntimeException("등록된 사용자를 찾을 수 없습니다.");
			}
		} catch (SQLException e) {
			e.printStackTrace(); // dump to log
			throw new RuntimeException("데이터베이스 오류가 발생하였습니다.(SQL-ERROR=" + e.getErrorCode() + ")");
